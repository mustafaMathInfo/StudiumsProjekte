...
const decryptAndVerifyMessage = async (item, email, privateKey, publicKey) => {
    const decryptPassword = await decryptPasswordWithPrivateKey(item.password, privateKey, publicKey, email);
    const isValidPassword =  await verifyMessageWithPublicKey(decryptPassword, item.detachedSignature, item.publicKeyArmoredSign)
    if (isValidPassword){
...
export {decryptPasswordWithPrivateKey, verifyMessageWithPublicKey, decryptAndVerifyMessage}
